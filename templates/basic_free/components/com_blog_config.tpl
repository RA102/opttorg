<form action="{$form_action}" method="post" name="cfgform" id="cfgform">
<div class="table-responsive">
 <table class="table table-striped">
	<tr>
	  <td width="40%"><strong>{$LANG.BLOG_TITLE}: </strong></td>
	  <td><input name="title" type="text" id="title" class="text-input" value="{$blog.title|escape:'html'}"/></td>
	</tr>
	<tr>
...
{if $cfg.seo_user_access || $is_admin}
    <tr>
        <td valign="middle"><strong>{$LANG.SEO_PAGETITLE}</strong><div class="hint">{$LANG.SEO_PAGETITLE_HINT}</div></td>
        <td>
            <input name="pagetitle" class="text-input" value="{$blog.pagetitle|escape:'html'}" />
        </td>
    </tr>
    <tr>
        <td valign="middle"><strong>{$LANG.SEO_METAKEYS}</strong></td>
        <td>
            <input name="meta_keys" class="text-input" value="{$blog.meta_keys|escape:'html'}" />
        </td>
    </tr>
    <tr>
        <td valign="middle"><strong>{$LANG.SEO_METADESCR}</strong><div class="hint">{$LANG.SEO_METADESCR_HINT}</div></td>
        <td>
            <textarea name="meta_desc" rows="3" class="text-input">{$blog.meta_desc|escape:'html'}</textarea>
        </td>
    </tr>
 {/if}
  </table>
  <table class="table  table-striped">
	<tr>
	  <td width="40%"><strong>{$LANG.BLOG_TYPE}: </strong></td>
	  <td>
		  <select name="ownertype" id="ownertype" onchange="selectOwnerType()" class="text-input">
  ...
  <table class="table table-condensed" id="multiblogcfg" style="display:{if $blog.ownertype=='single'}none;{else}block;{/if}">
	<tr>
	  <td width="40%"><strong>{$LANG.PREMODER_POST}: </strong></td>
	  <td>
		  <select name="premod" id="premod" class="text-input">
	...
	<tr>
	  ...
	  <td>
		  <select name="forall" id="forall" onchange="selectAuthorsType()" class="text-input">
  ...
  <table class="table table-condensed" id="multiuserscfg" style="margin:5px 0;display: {if $blog.ownertype=='single' || $blog.forall}none;{else}table;{/if}">
      <tr>
	  <td valign="middle" width="40%"><strong>{$LANG.CAN_WRITE_TO_BLOG}: </strong><br/>
		<select name="authorslist[]" size="15" multiple id="authorslist" class="text-input">
			{$authors_list}
		</select>
	  </td>
	  <td align="center">
	  ...
	  <td align="center" valign="middle"><strong>{$LANG.ALL_USERS}:</strong><br/>
		<select name="userslist" size="15" multiple id="userslist" class="text-input">
			{$users_list}
		</select>
	  </td>
	</tr>
