            ...
            <li class="media">
                ...
                <div class="media-body">
                    <h4 class="media-heading">
                        <a href="/shop/{$item.seolink}.html" title="{$item.art_no} / {$item.title}">{$item.art_no} / {$item.title}</a>
                    </h4>
                    <p class="trr">{if !$readonly}<span>
                            <input name="qty[{$item.cart_id}]" onchange="recountSumm()" style="width:50px;text-align:center" value="{if $item.cart_qty}{$item.cart_qty}{else}1{/if}" type="number"/>
                            ...
                            {$cfg.currency} {if $item.old_price}
                                <sup style="color:red;font-size:.8em;">{math equation="100-(x*100/y)" x=$iprice y=$item.old_price format="%.0f"}%</sup>{/if}
                        ...
                        {if !$readonly}
                            <a href="/shop/deletefromcart/{$item.cart_id}" title="Отказаться от товара" class="rejecting">
