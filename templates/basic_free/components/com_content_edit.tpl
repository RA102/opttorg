...
<form id="addform" name="addform" method="post" action="" enctype="multipart/form-data">
    ...
    <table class="proptable table table-striped">
        <tr>
            <td width="20%" valign="top">
                <strong>{$LANG.TITLE}:</strong>
            </td>
            <td valign="top">
                <input name="title" type="text" class="text-input" id="title" style="width:100%;" value="{$mod.title|escape:'html'}" />
            </td>
        </tr>
        <tr>
            <td valign="top">
                <strong>{$LANG.TAGS}:</strong><br />
                <span class="hint">{$LANG.KEYWORDS_TEXT}</span>
            </td>
            <td valign="top">
                <input name="tags" type="text" class="text-input" id="tags" style="width:100%;" value="{$mod.tags|escape:'html'}" />
        ...
        <tr>
            <td valign="top">
                ...
                <div><span class="hint">{$LANG.WHERE_LOCATE_ARTICLE}</span></div>
                {if $is_admin}
                    <div style="margin-top:10px"><span class="hint">{$LANG.FOR_ADD_ARTICLE_ON} <a href="/admin/index.php?view=tree">{$LANG.IN_CONFIG}</a> {$LANG.FOR_ADD_ARTICLE_ON_TEXT}</span></div>
                {/if}
            </td>
            <td valign="top">
        ...
        {if $cfg.img_users}
        <tr>
            <td valign="top" style="padding:10px;background:#f9f9f9;">
                <strong>{$LANG.IMAGE}:</strong>
            </td>
            <td valign="top" style="padding:10px;background:#f9f9f9;">
                {if $mod.image}
                    <div style="padding-bottom:10px">
                        <img src="/images/photos/small/{$mod.image}" width="36" style="float:left;border:#dedede 1px solid;padding:2px;border-radius:4px;margin-right:10px;" />
						<label for="delete_image" style="padding-top:7px;"><input type="checkbox" id="delete_image" name="delete_image" value="1" /> {$LANG.DELETE}</label>
                    </div>
