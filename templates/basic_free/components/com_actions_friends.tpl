{if $friends_total}
<div id="fr_body" class="actions-table">
<table border="0" cellspacing="0" cellpadding="0" class="action_friends">
  <tr>
    {if $page > 1}
    ...
    <td width="85px" id="fr0" {if !$user_id}class="selected"{/if}>
    	<div class="action_fr">
    		<a href="javascript:void(0);" onclick="selectUser(0);" title="{$LANG.ALL_FRIENDS}"><img class="media-object" border="0" src="/templates/{template}/images/actions_people.png"  /></a>
        </div>
    </td>
    {/if}
    {foreach key=tid item=friend from=$friends}
        <td width="85px" id="fr{$friend.id}" {if $user_id == $friend.id}class="selected"{/if}>
            <div class="action_fr">
                <a href="javascript:void(0);" onclick="selectUser({$friend.id});" title="{$friend.nickname|escape:'html'}"><img border="0" class="media-object" src="{$friend.avatar}" /></a>
            </div>
        </td>
    {/foreach}
    {if $page != $total_pages}
