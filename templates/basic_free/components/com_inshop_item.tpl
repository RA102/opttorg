...
<h1 class="con_heading" itemprop="name"><a href="#" class="history-back hidden-lg hidden-md" onclick="history.back();">&laquo;</a> {$item.title}</h1>
...
<div class="good-wrp">	
		<form action="/shop/addtocart" method="POST">	
			<div class="row">
				<div class="col-md-4 col-sm-12">
					<div class="thumb-cat">
						<a id="itemimage" href="/images/photos/medium/{$item.filename}" class="lightbox-enabled" rel="lightbox-galery" title="{$item.title|escape:'html'} - {$item.art_no}"><img src="/images/photos/small/{$item.filename}" class="img-resp" alt="{$item.title|escape:'html'} - {$item.art_no} – интернет-магазин SanMarket" itemprop="image" />{if $item.old_price>0}
			{assign var="disco" value=((100-($iprice*100/$item.old_price))|ceil)}<div class="ribbon-lt"><span>Скидка {$disco}%</span></div>{/if}
			{if $item.novinka==1}<div class="ribbon-rt"><span>Новинка</span></div>{/if}			
			{if $item.is_hit}<div class="ribbon-rb"><span>Хит</span></div>{/if}
			{if $item.is_spec}<div class="ribbon-lb"><span>Акция</span></div>{/if}</a>
					</div>
        ...
        {if $item.images}
            ...
            {foreach key=num item=file from=$item.images}
					<li>
						<a href="/images/photos/medium/{$file}" class="lightbox-enabled" rel="lightbox-galery" title="{$item.title|escape:'html'} - {$item.art_no}, фото {$num+1}"><img src="/images/photos/small/{$file}" class="img-responsive" alt="{$item.title|escape:'html'} - {$item.art_no}, фото {$num+1} - интернет-магазин SanMarket" /></a>
					</li>			
            {/foreach}
				</ul>
            </div>
				...
				<div class="col-md-8 col-sm-12">
					<div class="chars-wrp">
						<div class="row">
							...
							<div class="col-md-5 col-md-push-7" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
				{if $iprice > 0}
                {if $cfg.is_shop && !$item.hide_price}
                                {if !$cfg.track_qty || $qty || $qty_from_vendor}
									{if $item.old_price > 0}<div class="old-price"><s>{$item.old_price|number_format:0:" ":" "} {$cfg.currency}</s></div>{/if}
									...
									<div class="new-price"><span id="results">{$iprice|number_format:0:" ":" "}</span> {$cfg.currency}</div>
									<div>
{*										{if $qty > 1 }*}
{*											<p class="count-item">Есть в наличии {$qty} шт.</p>*}
{*										{elseif $qty_from_vendor}*}
{*											<p class="count-item"> Под заказ {$qty_from_vendor} шт.</p>*}
{*										{/if}*}
									</div>
									{if $cfg.track_qty}
									...
									<input type="hidden" name="var_art_no" value="" />
										...
										{if $item.kaspikz}<div class="small mt10 text-center btn-kaspi"><a rel="nofollow" target="_blank" href="{$item.kaspikz}"><img src="/templates/basic_free/img/kaspykz.png" height="48" /></a></div>{/if}
									<div id="dynamic"></div>
									...
									<div class="h4 text-right">Код товара: {$item.art_no}{if $item.ven_code}<br />Код производителя: {$item.ven_code}{/if}</div>
									<p class="text-right text-small"><strong>При заказе по телефону</strong><br /> назовите менеджеру данный код</p>
									<div class="text-right"><img src="/templates/{template}/img/kaspired.jpg" class="kaspired" /> <img src="/templates/{template}/img/visa.jpg" class="visa" /><div class="smallest">Kaspi Red 0% только в Караганде</div></div>
                                {else}
				...
				{/if}	
					<div class="average text-right">{section name=foos start=1 loop=6 step=1}<span class="glyphicon glyphicon-star"{if $item.rating < $smarty.section.foos.index} style="color:lightgray"{/if}></span>{/section}<br /><a href="#cmms" class="cmmlink">({$item.rating_votes|spellcount:'отзыв':'отзыва':'отзывов'})</a></div>			
							...
							<div class="col-md-7 col-md-pull-5">
								...
								{if $cfg.show_vendors && $item.vendor}<div class="char-div">Производство: <span class="pull-right" itemprop="brand">{$item.vendor}</span></div>{/if}
								{if $item.ves>0}<div class="char-div">Вес: <span class="pull-right">{$item.ves} кг.</span></div>{/if}
								{if $item.vol>0}<div class="char-div">Объём: <span class="pull-right">{$item.vol} м<sup>3</sup></span></div>{/if}
								{if $item.chars}
								...
								{/if}
								{if $item.description}
									<br /><article class="item-description" itemprop="description">{$item.description}</article>
									
								{/if}
								
							</div>							
							
							<div class="mobile-tocart hidden-lg hidden-md hidden-sm">
								<div class="mob-title"><img src="/images/photos/small/{$item.filename}" /> Товар: {$item.title}{if $iprice > 0} - {$iprice|number_format:0:" ":" "} {$cfg.currency}{/if}</div>
								<table width="100%" border="0"><tr>{if $iprice > 0}<td width="50%"><button type="submit" class="btn btn-main btn-block{if $item.is_in_cart>0} btn-disabled{/if}">{if $item.is_in_cart>0}В корзине{else}{if $item.qty!=0}В корзину{else}В корзину{/if}{/if}</button></td>{/if}<td><a class="btn btn-whapp btn-block" href="https://wa.me/77775409927?text=Я%20заинтересован%20в%20покупке%20товара%20№{$item.art_no}%20https://sanmarket.kz/shop/{$item.seolink}.html">Заказ через Whatsapp</a></td></tr></table>
							</div>
						</div>
					</div>
				</div>
...
{if $sims}
<div class="h3 main-color">С этим товаром также покупают:</div><br />
<div class="row">
				...
				<div class="col-md-3 col-sm-6">
					<div class="media">
						...
						<div class="media-body">
							...
							<p>{if $sim.old_price}<s>{$sim.old_price|number_format:0:" ":" "}</s>{/if} {$sprice|number_format:0:" ":" "} тг.</p>
						</div>
					</div>
				</div>	
        {/foreach}
...
{/if}	
<div class="h3 main-color">Ещё в данной рубрике:</div><br />
<div class="row">
				...
				<div class="col-md-3 col-sm-6">
					<div class="media">
						...
						<div class="media-body">
							...
							<p>{if $rel.old_price}<s>{$rel.old_price|number_format:0:" ":" "}</s>{/if} {$sprice|number_format:0:" ":" "} тг.</p>
						</div>
					</div>
				</div>	
        {/foreach}
    ...
    <div class="modal-content">
		...
		<div class="modal-body">
			...
			<table width="100%" border="0">
				<tr>
					<td valign="middle" width="80"><input style="width:80px !important;" class="form-control" id="qtyy1" name="qtyy1" type="number" min="1" value="1" oninput="change1()" /></td>
					<td valign="middle" class="text-right"><strong><span id="results1">{$iprice|number_format:0:" ":" "}</span> {$cfg.currency}</strong></td>
				</tr>
			</table><br />
