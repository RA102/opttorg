...
if ($opt == 'show_comment'){
	...
	$cmm = $inDB->get_fields('cms_comments', "id = '$item_id'", '*');
	$trg = $cmm['target_id'];
	$shp = $inDB->get_fields('cms_shop_items', "id = '$trg'", '*');
	$rtg = (floatval($cmm['rating1']) + floatval($cmm['rating2']) + floatval($cmm['rating3']) + floatval($cmm['rating4'])) / 3;
	if ($shp['rating']==0) {
		$avrg = ceil(floatval($rtg));
	} else {
		$avrg = ceil((floatval($shp['rating']) + floatval($rtg)) / 2); 
	}
	$votes = $shp['rating_votes'] + 1;
	$inDB->query("UPDATE cms_shop_items SET rating = '$avrg', rating_votes = '$votes' WHERE id = '$trg'");
    echo '1'; exit;
}
