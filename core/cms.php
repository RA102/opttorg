...
class cmsCore {
    ...
    private static function detectContext(){
        if((isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest') || isset($_SERVER['HTTP_X_PJAX'])) {
            self::$is_ajax = true;
        } else {
            self::$is_ajax = false;
        }
    ...
    public static function strClear($input, $strip_tags=true){
        ...
        $string = stripslashes($string); //(@get_magic_quotes_gpc()) ? stripslashes($string) : $string;
        $string = rtrim($string, ' \\');
        if ($strip_tags) {
