            ...
            foreach ($filter as $key => $val) {
                ...
                if ($val && $key) {
                    ...
                    if (!is_array($val)) {
                        ...
                        switch ($key) {
                            case 'pfrom':
                                $model->wherePriceFrom($val);
                                break;
                            case 'pto':
                                $model->wherePriceTo($val);
                                break;
                            default:
                                if ($chars[$key]['values']) {
...
//        $model->orderByItemsWithPriceAndCount();
        ...
        foreach (array('ordering', 'price', 'title', 'id') as $order) {
            ...
            switch ($order) {
                case 'ordering':
                    $name = $_LANG['SHOP_SORT_ORDERING'];
                    break;
                case 'price':
                    $name = $_LANG['SHOP_SORT_PRICE'];
                    break;
                case 'title':
                    $name = $_LANG['SHOP_SORT_TITLE'];
                    break;
                case 'id':
                    $name = $_LANG['SHOP_SORT_DATE'];
                    break;
            }

            $order_types[] = array('order' => $order, 'name' => $name, 'selected' => ($orderby == $order));
...
//        $items = array_splice($tmp, 0, count($tmp));
        ...
        if (!$root_cat['tpl']) {
            $root_cat['tpl'] = 'com_inshop_view.tpl';
        }
...
//============================================================================//
    ...
    if ($do == 'download') {
        ...
        if (!file_exists(PATH . $file)) {
            $inCore->halt('Файл не найден. Обратитесь в службу поддержки магазина. Номер вашего заказа: #' . $item['order_id']);
        }
...
//============================================================================//
    ...
    if ($do == 'view_vendor') {
        ...
        if ($page == 1) {
            $is_pager = 1;
            $inPage->addHead('<link rel="canonical" href="https://sanmarket.kz/shop/vendors/' . $vendor_id . '" />');
        } else {
            $is_pager = 0;
        }
...
//            $order['d_price']       = $delivery_types[$d_type]['price'];
        ...
        if (!$order['customer_name'] && in_array('name', $cfg['ord_req'])) {
            $errors = true;
            cmsCore::addSessionMessage($_LANG['SHOP_ERR_REQ_NAME'], 'error');
        }
        if (!$order['customer_phone'] && in_array('phone', $cfg['ord_req'])) {
            $errors = true;
            cmsCore::addSessionMessage($_LANG['SHOP_ERR_REQ_PHONE'], 'error');
        }
        if (!$order['customer_email'] && in_array('email', $cfg['ord_req'])) {
            $errors = true;
            cmsCore::addSessionMessage($_LANG['SHOP_ERR_REQ_EMAIL'], 'error');
        }
        if (!$order['customer_address'] && in_array('address', $cfg['ord_req'])) {
            $errors = true;
            cmsCore::addSessionMessage($_LANG['SHOP_ERR_REQ_ADDRESS'], 'error');
        }
        if ($customer_type == 'org') {
            if (!$order['customer_org'] && in_array('org', $cfg['ord_req'])) {
                $errors = true;
                cmsCore::addSessionMessage($_LANG['SHOP_ERR_REQ_ORG'], 'error');
            }
            if (!$order['customer_inn'] && in_array('inn', $cfg['ord_req'])) {
                $errors = true;
                cmsCore::addSessionMessage($_LANG['SHOP_ERR_REQ_INN'], 'error');
            }
        }
...
//============================================================================//
    ...
    if ($do == 'process_payment') {
        ...
        if (!$psys_id || mb_strstr($psys_id, '.')) {
            $inCore->halt("ERR: НЕВЕРНАЯ ПЛАТЕЖНАЯ СИСТЕМА");
        }

        $psys = $model->getPaymentSystem($psys_id);

        if (!$psys) {
            $inCore->halt("ERR: НЕИЗВЕСТНАЯ ПЛАТЕЖНАЯ СИСТЕМА: $psys_id");
        }
        ...
        if (!$order_id) {
            $order_id = $inCore->request('InvId', 'int', 0);
        }
...
//============================================================================//
    ...
    if ($do == 'order_success') {
        ...
        if (!$order_id) {
            $order_id = $inCore->request('ik_payment_id', 'int', 0);
        }

        //интеркасса 2.0 возвращает ik_pm_no в качестве номера заказа
        if (!$order_id) {
            $order_id = $inCore->request('ik_pm_no', 'int', 0);
        }
        ...
        if ($order_items) {
            ...
            $showorders .= '</table>';
            if ($order['giftcode'] > 0) {
                $itogo = round($order['summ'] - ($order['summ'] * $order['giftcode'] / 100), 0);
                $gift = ', со скидкой ' . $order['giftcode'] . '% = ' . $itogo . ' kzt';
            }
...
//============================================================================//
    ...
    if ($do == 'order_accept') {
        ...
        if ($order) {
            $model->sendOrder($order, 'accept', $cfg);
        }
...
//============================================================================//
    ...
    if ($do == 'sberbank') {
        ...
        if ($order['summ_parts'][1] == '') {
            $order['summ_parts'][1] = '00';
        }
...
//============================================================================//
    ...
    if ($do == 'balance') {
        ...
        if ($balance < $summ) {
            ...
            $billing_ticket = array('action' => $comment, 'cost' => $summ, 'amount' => $summ - $balance, 'url' => $_SERVER['REQUEST_URI']);
...
//============================================================================//
    ...
    if ($do == 'export_yml') {
        ...
        if (!isset($cfg['yml']['store'])) {
            $cfg['yml']['store'] = -1;
        }
        if (!isset($cfg['yml']['pickup'])) {
            $cfg['yml']['pickup'] = -1;
        }
        if (!isset($cfg['yml']['delivery'])) {
            $cfg['yml']['delivery'] = -1;
        }
