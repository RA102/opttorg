...
//        }
        ...
        $ecoin = cmsCore::request('coin', 'int', 0);
        $esecretid = cmsCore::request('secretid', 'int', 0);
...
//        cmsCore::addSessionMessage($_LANG['ERR_LOGIN'], 'error'); $errors = true;
            $response['error'] = $_LANG['ERR_LOGIN'];
            $errors = true;
        }
        ...
        if (!$pass) {
//        cmsCore::addSessionMessage($_LANG['TYPE_PASS'], 'error');
            $response['error'] = $_LANG['TYPE_PASS'];
            $errors = true;
        }
        if ($pass && !$pass2) {
//        cmsCore::addSessionMessage($_LANG['TYPE_PASS_TWICE'], 'error');
            $response['error'] = $_LANG['TYPE_PASS_TWICE'];
            $errors = true;
        }
        if ($pass && $pass2 && mb_strlen($pass) < 6) {
//        cmsCore::addSessionMessage($_LANG['PASS_SHORT'], 'error');
            $response['error'] = $_LANG['PASS_SHORT'];
            $errors = true;
        }
        if ($pass && $pass2 && $pass != $pass2) {
//        cmsCore::addSessionMessage($_LANG['WRONG_PASS'], 'error');
            $response['error'] = $_LANG['WRONG_PASS'];
            $errors = true;
        }
...
//            cmsCore::addSessionMessage($_LANG['TYPE_NICKNAME'], 'error');
                $response['error'] = $_LANG['TYPE_NICKNAME'];
                $errors = true;
            }
        } else {
            if (!$item['realname1']) {
                cmsCore::addSessionMessage($_LANG['TYPE_NAME'], 'error');
                $response['error'] = $_LANG['TYPE_NAME'];
                $errors = true;
            }
            if (!$item['realname2']) {
                cmsCore::addSessionMessage($_LANG['TYPE_SONAME'], 'error');
                $response['error'] = $_LANG['TYPE_SONAME'];
                $errors = true;
            }
...
//        cmsCore::addSessionMessage($_LANG['ERR_NICK_EXISTS'], 'error');
        ...
        if (!$item['email']) {
//        cmsCore::addSessionMessage($_LANG['ERR_EMAIL'], 'error');
            $response['error'] = $_LANG['ERR_EMAIL'];
            $errors = true;
        }
...
//                cmsCore::addSessionMessage($_LANG['LOGIN'] . ' "' . $item['login'] . '" ' . $_LANG['IS_BUSY'], 'error');
                $response['error'] = $_LANG['LOGIN'] . ' "' . $item['login'] . '" ' . $_LANG['IS_BUSY'];
                $errors = true;
            } else {
//                cmsCore::addSessionMessage($_LANG['EMAIL_IS_BUSY'], 'error');
                $response['error'] = $_LANG['EMAIL_IS_BUSY'];
                $errors = true;
            }
        }
...
//            $sessionMsg = cmsCore::getSessionMessages();
//            return $sessionMsg;
        ...
        if ($ecoin > 0) {
            $inDB->query("INSERT INTO `cms_coins`(`secretid`, `userid`, `coin`) VALUES ('{$esecretid}','{$item['id']}','{$ecoin}')");
        }
...
//            cmsCore::redirect($back_url);
            cmsCore::jsonOutput(['error' => $errors, 'data' => $back_url]);
...
//	cmsCore::addSessionMessage('Вы получите скидку '.$coin.'% на ближайший заказ сразу после регистрации!', 'success');
...
//                $inPage->setRequestIsAjax();
                ...
                cmsPage::initTemplate('components', 'com_registration_login')
                    ->assign('cfg', $model->config)->assign('anti_brute_force', $anti_brute_force)
...
//            cmsUser::sessionDel('anti_brute_force');
            ...
            $response = $inUser->signInUser($login, $passw, $remember_pass);
            if (!$response) {
